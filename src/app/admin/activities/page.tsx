...
export default function AdminActivitiesPage() {
  ...
  const fetchItems = async (page: number = 1, limitParam: number = 10, keywordParam?: string) => {
    try {
      ...
      const res = await fetch(`${API_URL}/activities?${query}`, { credentials: 'include' });
      ...
      setItems((data || []).map((x: any) => ({
        ...
        registration: x.registration ? { id: x.registration.id } : null,
      })));
  ...
  return (
    <div className='p-6'>
      <div className='flex justify-between items-center mb-4'>
        ...
        <AddEditActivityDialog mode='add' onSave={handleAdd} />
      </div>
      <div className='flex gap-4 mb-4'>
        <Input
          placeholder='Search by plate or revenue type'
          value={searchKeyword}
      ...
      <AdminActivitiesTable items={items} onEdit={handleEdit} onDelete={handleDelete} />
      <PaginationWrapper

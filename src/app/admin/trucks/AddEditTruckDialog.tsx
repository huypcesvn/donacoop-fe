...
export default function AddEditTruckDialog({
  ...
  const [form, setForm] = useState({
    ...
    weighingMethod: initialData?.weighingMethod || '',
    weighingPosition: initialData?.weighingPosition || '',
    allowedLoad: initialData?.allowedLoad ?? undefined,
  ...
  return (
    ...
    >
      <AlertDialogTrigger asChild>
        {children ?? <Button>{mode === 'add' ? 'Add Truck' : 'Edit'}</Button>}
      </AlertDialogTrigger>

      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{mode === 'add' ? 'Add Truck' : 'Edit Truck'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div className='gap-3 grid grid-cols-1 md:grid-cols-2 mt-4'>
          ...
          <Input placeholder='Group' value={form.group} onChange={(e) => setForm({ ...form, group: e.target.value })} />
          <Input placeholder='Weighing Method' value={form.weighingMethod} onChange={(e) => setForm({ ...form, weighingMethod: e.target.value })} />
          <Input placeholder='Weighing Position' value={form.weighingPosition} onChange={(e) => setForm({ ...form, weighingPosition: e.target.value })} />
          <Input placeholder='Allowed Load' type='number' value={form.allowedLoad ?? ''} onChange={(e) => setForm({ ...form, allowedLoad: e.target.value ? Number(e.target.value) : undefined })} />

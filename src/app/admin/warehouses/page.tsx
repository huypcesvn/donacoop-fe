...
import AdminWarehousesTable from './AdminWarehousesTable';
...
export default function AdminWarehousesPage() {
  ...
  const fetchItems = async (page: number = 1, limitParam: number = 10, keywordParam?: string) => {
    try {
      ...
      const res = await fetch(`${API_URL}/warehouses?${query}`, { credentials: 'include' });
      const { data, total, page: resPage, limit: resLimit, totalPages: resTotalPages } = await res.json();
      setItems((data || []).map((x: any) => ({ id: x.id, name: x.name, company: x.company ? { id: x.company.id, name: x.company.name } : null })));
      ...
      console.error(err);
      toast('Failed to fetch warehouses.');
    }
  };
  ...
  return (
    <div className='p-6'>
      <div className='flex justify-between items-center mb-4'>
        <h1 className='font-bold text-3xl'>Warehouses</h1>
        <AddEditWarehouseDialog mode='add' onSave={handleAdd} />
      </div>
      ...
      <AdminWarehousesTable items={items} onEdit={handleEdit} onDelete={handleDelete} />
      <PaginationWrapper
